...
@router.message(F.text == "/upload_mythai")
...
@router.message(F.text == "/upload_ustritso")
...
async def upload_photo(message: Message):
    ...
    if not business:
        await message.answer("❗ Выбери бизнес: /upload_mythai или /upload_ustritso")
        return

    file_id = message.photo[-1].file_id
    tags = (message.caption or "").lower()

    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO content (business, file_id, tags) VALUES (?, ?, ?)",
            (business, file_id, tags)
        )
        await db.commit()
